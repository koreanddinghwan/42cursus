...
typename std::deque<int>::iterator PmergeMe::binary_search_deque(std::deque<int>::iterator begin, std::deque<int>::iterator end, const int& value) 
{
...
void PmergeMe::merge_deque(std::deque<int> &input_deque)
{
	...
	// 입력 배열을 두 개의 부분 배열로 분할
	middle= input_deque.begin() + input_deque.size() / 2;
	...
	while (i != left.end() && j != right.end()) 
	{
		...
		{
			temp.insert(binary_search_deque(right.begin(), j, *i), *i);
			++i;
		}
	}
...
void PmergeMe::insertion_deque(std::deque<int> &input_deque)
{
	for (std::deque<int>::iterator i = input_deque.begin(); i != input_deque.end(); ++i)
	{
		...
		while (j != input_deque.begin() && *std::prev(j) > *j)
		{
